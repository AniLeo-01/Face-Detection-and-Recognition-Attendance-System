...
from mtcnn.network import NetworkFactory

__author__ = 'Aniruddha Mandal'

class StageStatus(object):
    """update status of each MTCNN stages"""
    ...
    def update(self, pad_result: tuple):
        self.dy, self.edy, self.dx, self.edx, self.y, self.ey, self.x, self.ex, self.tmpw, self.tmph = pad_result

class MTCNN(object):
    """Allow to perform MTCNN detection:
        a) Detection of faces (with the confidence probability)
        b) Detection of keypoints (left eye, right eye, nose, left mouth corner, right mouth corner)
    """
    def __init__(self, weights_file: str = None, min_face_size: int = 20, steps_threshold: list = None, scale_factor: float = 0.709):
        ...
        Args:
            weights_file: file uri with the weights of the P, R, O networks from MTCNN.
                default: load the mtcnn_weights.npy package
            min_face_size: minimum size of the face to detect
            steps_threshold: step's threshold values
        ...
        self._pnet, self._rnet, self._onet = NetworkFactory().load_net(weights_file)
    ...
    def min_face_size(self, mfc=20):
        try:
            ...
            self._min_face_size = 20 #incase of ValueError the default value 20 is set to mfc
    ...
    def __scale_image(image, scale: float):
        ...
        Args:
            ...
            scale: scale factor
        Returns:
            im_data_normalized: normalized image
        """
        height, width, _ = image.shape
        width_scaled = int(np.ceil(width*scale))
        height_scaled = int(np.ceil(height*scale))

        im_data = cv2.resize(image, (width_scaled, height_scaled), interpolation = cv.INTER_AREA)
    ...
    def __generate_bounding_box(imap, reg, scale, t):
        #use heatmap to generate bounding box
        stride = 2
        cellsize = 12
    ...
    def __nms(boxes, threshold, method):
        ...
        Args:
            boxes: np array with bounding boxes
            threshold: threshold of nms
            method: nms method ('min', 'union')
        """
        if boxes.size == 0:
            return np.empty((0,3))
        ...
        area = (x2 - x1 + 1) * (y2 - y1 + 1) #area of rectangle
        sorted_scores = np.argsort(scores)

        pick = np.zeros_like(scores, dtype = np.int16) #empty bbox array for nms output
        counter = 0
        while sorted_scores.size > 0:
            i = sorted_scores[-1]
            pick[counter] = i
            counter += 1
            idx = sorted_scores[0: -1]
            ...
            if method is 'Min':
                ovr = inter / np.maximum(area[i], area[idx])
            else:
                ovr = inter / (area[i] + area[idx] - inter)
            
            sorted_scores = sorted_scores[np.where(ovr <= threshold)]
    ...
    def __pad(total_boxes, w, h):
        #compute the padding coordinates (pad the bboxes to square)
    ...
    def detect_faces(self, img) -> list:
        """Detects bounding box from the image
        Args:
            img: image input
        Returns:
            list containing the bounding box detected with the keypoints
        """
        ...
        #piping each stages
        for stage in stages:
            result = stage(img, result[0], result[1]) #(img, scales, stage_status)
    ...
    def __stage1(self, image, scales: list, stage_status: StageStatus):
        ...
        if numboxes > 0:
            ...
            status = StageStatus(self.__pad(total_boxes.copy(), stage_status.width, stage_status.height, width = stage_status.width,
    ...
    def __stage2(self, img, total_boxes, stage_status: StageStatus):
        ...
        Args:
            img: Input image
            total_boxes: bboxes
            stage_status: staging parameters of the bboxes
        Returns:
    ...
    def __stage3(self, img, total_boxes, stage_status: StageStatus):
        ...
        Args:
            ...
            stage_status: staging parameters of bounding_boxes
        Returns: 
            total_boxes, points
        """
        num_boxes = total_boxes.shape[0]
